...
// Read blockPlaceDelay per-call from bot._config so each bot instance uses its
// own profile config. Replaces the runtimeConfig singleton that broke in
// multi-bot scenarios (module-level constant captured the first bot's value).
function blockDelay(bot) {
  return bot._config?.skills?.blockPlaceDelay ?? 0;
}
...
async function wait(bot, milliseconds) {
    /**
...
async function collectBlock(bot, blockType, num = 1, exclude = null) {
    /**
...
async function breakBlockAt(bot, x, y, z) {
    /**
    ...
    if (block.name !== 'air' && block.name !== 'water' && block.name !== 'lava') {
        if (bot.modes.isOn('cheat')) {
            { const d = blockDelay(bot); if (d > 0) await new Promise(r => setTimeout(r, d)); }
...
async function placeBlock(bot, blockType, x, y, z, placeOn = 'bottom', dontCheat = false) {
    /**
...
async function putInChest(bot, itemName, num = -1) {
    /**
...
async function takeFromChest(bot, itemName, num = -1) {
    /**
...
async function goToPosition(bot, x, y, z, min_distance = 2) {
    /**
...
async function goToNearestBlock(bot, blockType, min_distance = 2, range = 64) {
    /**
...
async function goToNearestEntity(bot, entityType, min_distance = 2, range = 64) {
    /**
...
async function goToPlayer(bot, username, distance = 3) {
    /**
...
async function moveAway(bot, distance) {
    /**
...
async function moveAwayFromEntity(bot, entity, distance = 16) {
    /**
...
async function avoidEnemies(bot, distance = 16) {
    /**
...
async function stay(bot, seconds = 30) {
    /**
...
async function useDoor(bot, door_pos = null) {
    /**
...
async function tillAndSow(bot, x, y, z, seedType = null) {
    /**
...
async function activateNearestBlock(bot, type) {
    /**
 ...
 */
async function showVillagerTrades(bot, id) {
 ...
 */
async function tradeWithVillager(bot, id, index, count) {
    const villagerEntity = await findAndGoToVillager(bot, id);
    if (!villagerEntity) {
        return false;
    }
...
async function digDown(bot, distance = 10) {
    /**
...
async function goToSurface(bot) {
    /**
...
async function useToolOn(bot, toolName, targetName) {
    /**
...
async function useToolOnBlock(bot, toolName, block) {
    /**
...
module.exports = { log, craftRecipe, wait, smeltItem, clearNearestFurnace, attackNearest, attackEntity, defendSelf, collectBlock, pickupNearbyItems, breakBlockAt, placeBlock, equip, discard, putInChest, takeFromChest, viewChest, consume, giveToPlayer, goToGoal, goToPosition, goToNearestBlock, goToNearestEntity, goToPlayer, followPlayer, moveAway, moveAwayFromEntity, avoidEnemies, stay, useDoor, goToBed, tillAndSow, activateNearestBlock, showVillagerTrades, tradeWithVillager, digDown, goToSurface, useToolOn }
