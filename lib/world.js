...
function getNearbyPlayers(bot, maxDistance) {
...
// Opens a spawner block's GUI (right-click) and reads the window title sent by
// the DonutSMP spawner plugin.  The server encodes the stack count there:
//
//   window.title (prismarine-nbt compound):
//     { type:'compound', value:{ text:{ type:'string', value:'2 ѕᴋᴇʟᴇᴛᴏɴ ѕᴘᴀᴡɴᴇʀѕ' }, ... } }
//
// Confirmed by live probe: NBT tile-entity has no custom stack field.
//
// @param  {Bot}    bot         mineflayer bot
// @param  {Block}  block       spawner block from bot.blockAt() or getNearestBlock()
// @param  {number} timeoutMs   ms to wait for windowOpen (default 5000)
// @returns {number|null}  stack count, or null if title could not be read/parsed
async function getSpawnerStackCount(bot, block, timeoutMs = 5000) {
    let window
    try {
        window = await new Promise((resolve, reject) => {
            const timer = setTimeout(
                () => reject(new Error(`windowOpen not received within ${timeoutMs}ms`)),
                timeoutMs
            )
    ...
    // Extract the plain text value from the prismarine-nbt compound title.
    // Shape: { type:'compound', value:{ text:{ type:'string', value:'N MOB spawners' }, ... } }
    let text = ''
...
module.exports = { getNearestFreeSpace, getBlockAtPosition, getSurroundingBlocks, getFirstBlockAboveHead, getNearestBlocks, getNearestBlocksWhere, getNearestBlock, getNearbyEntities, getNearestEntityWhere, getNearbyPlayers, getVillagerProfession, getInventoryCounts, getCraftableItems, getPosition, getNearbyEntityTypes, isEntityType, getNearbyPlayerNames, getNearbyBlockTypes, isClearPath, shouldPlaceTorch, getBiomeName, getSpawnerStackCount }
