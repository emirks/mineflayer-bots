...
// @param {boolean} [opts.debug=false]           when true: log all window slots + dump JSON files
...
const { resolveComponent, getDisplayName, getLore, findSlotByKeyword, normalizeText,
        parseMoneyString, formatMoney }                                                             = require('./nbtParse')
...
// Re-exported at the bottom of this file for callers that imported them from here.
...
function getActiveSortMode(sortSlot) {
    ...
    for (const rawLine of loreList) {
        try {
            ...
            // Flat format: color on root (used by some DonutSMP buttons)
            if (comp.color && comp.color !== 'white' && comp.color !== 'gray' && comp.color !== 'dark_gray') {
                return (comp.text ?? '').replace(/^[•\s]+/, '').trim()
            }

            // Nested format: color on extra child (used by order sort buttons)
...
async function deliverOneOrder(bot, opts = {}) {
    ...
    if (debug && runDir) dumpWindowToFile(bot, ordersWin, 'orders_page1')
    ...
    if (debug && runDir) dumpWindowToFile(bot, deliverWin, 'orders_deliver_items')
    ...
    if (debug && runDir) dumpWindowToFile(bot, confirmWin, 'orders_confirm_delivery')
