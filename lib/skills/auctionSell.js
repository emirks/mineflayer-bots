...
const { resolveComponent, getDisplayName, getLore, normalizeText } = require('./nbtParse')
...
async function auctionSellAll(bot, opts = {}) {
    ...
    while (true) {
        ...
        // Market prices may have moved during the pause; we break both loops and
        // restart from step A (fresh /ah price check) before listing again.
        let needsPriceRecheck = false

        for (const winSlot of hotbarSlotNums) {
            if (bot._quitting || needsPriceRecheck) break
            ...
            while (true) {
                ...
                if (state.limitHit) {
                    ...
                    try {
                        ...
                        bot.log.warn(`${LOG} ${err.message} — aborting sell loop`)
                        bot._quitting = true
                        break
                    }
                    // Market may have moved while we were waiting — abort current
                    // batch and re-check the AH price before listing the remaining
                    // hotbar slots.  Items still in hotbar will be found by the
                    // next fillHotbarWith1x (no moves, just re-reads them).
                    bot.log.info(`${LOG} Sale received — re-fetching AH price before next listing`)
                    needsPriceRecheck = true
                    break   // break inner while; outer for checks needsPriceRecheck
                }
        ...
        // ── E. Batch summary / price-recheck restart ──────────────────────────
    ...
    return { totalSold, batches: batchNum, totalEarned: state.totalEarned, buyCount: state.buyCount }
}
