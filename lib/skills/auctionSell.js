...
async function auctionSellAll(bot, opts = {}) {
    ...
    // Shared mutable state updated by the chat watcher running in the background
    const state = {
        limitHit:   false,   // true while the auction listing cap is active
        totalEarned: 0,      // cumulative $-value of all confirmed sales this run
        buyCount:    0,      // number of completed sales
    }
    ...
    while (true) {
        ...
        const lowestPrice = await getAuctionLowestPrice(bot, searchTerm, { winTimeoutMs, clickDelayMs, debug })

        if (lowestPrice === null) {
            bot.log.warn(`${LOG} Cannot determine lowest price — stopping`)
            break
        }
        ...
        // ── D. Sell each hotbar slot ──────────────────────────────────────────
        for (const winSlot of hotbarSlotNums) {
            if (bot._quitting) break

            const hbIdx = winSlot - 36   // 0–8

            // Re-read the slot live — might have changed (e.g. purchased by a player)
            const item = bot.inventory.slots[winSlot]
            if (!item || item.name !== itemName || item.count !== 1) {
                bot.log.info(`${LOG} Hotbar slot ${winSlot} no longer has 1×${itemName} — skipping`)
                continue
            }

            // Select hotbar slot — sends held_item_change only, NO window_click
            bot.log.info(`${LOG} Equipping hotbar ${hbIdx} (window slot ${winSlot})`)
            bot.setQuickBarSlot(hbIdx)
            await sleep(300)

            // Sanity-check held item
            ...
            // Register listener BEFORE the chat command — windowOpen event can never be missed
            const winPromise = waitForWindowOpen(bot, winTimeoutMs)
            ...
            try {
                ...
                bot.log.warn(`${LOG} CONFIRM LISTING window did not open: ${err.message}`)
                if (bot.currentWindow) { try { bot.closeWindow(bot.currentWindow) } catch { } }
                await sleep(clickDelayMs)
                continue
            }

            const confirmed = await clickConfirmInListingWindow(bot, confirmWin, clickDelayMs, debug)

            // Verify the held slot is now empty (item left hand = listed successfully)
            await sleep(400)
            const heldAfter = bot.inventory.slots[bot.quickBarSlot + 36]
            if (!heldAfter) {
                if (confirmed) totalSold++
                bot.log.info(`${LOG} ✓ Listed 1×${itemName} @ ${formatMoney(targetPrice)}  (total listed: ${totalSold})`)
            } else {
                bot.log.warn(
                    `${LOG} Slot ${winSlot} still has ${heldAfter.name}×${heldAfter.count}` +
                    ` after confirm — item may NOT have been listed`
                )
            }
    ...
    bot.log.info(`${LOG} ═══ Done: listed ${totalSold}×${itemName} across ${batchNum} batch${batchNum !== 1 ? 'es' : ''} ═══`)
    return { totalSold, batches: batchNum }
}
