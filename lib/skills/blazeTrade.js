...
async function deliverToOrder(bot, opts = {}) {
    ...
    // ── Click order → Deliver Items window ────────────────────────────────────
    // KEY WINDOW: time from "found order" to confirm is the most critical metric
    T.tOrderClick = Date.now()
    let deliverWin
    try {
        const p = waitForWindowOpen(bot, winTimeoutMs)
    ...
    bot.log.perf(`${LOG} ⏱ order-click → confirm-click (TOTAL EXPOSURE): ${ms(T.tOrderClick)}`)
